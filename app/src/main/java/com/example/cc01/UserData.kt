...
object UserData {
    ...
    // signed in status
    private val SignedIn = MutableLiveData<Boolean>(false)
    var isSignedIn: LiveData<Boolean> = SignedIn

    fun setSignedIn(newValue : Boolean) {
        // use postvalue() to make the assignation on the main (UI) thread
        SignedIn.postValue(newValue)
    }
    ...
    fun resetNotes() {
        _notes.value?.clear()  //used when signing out
        _notes.notifyObserver()
    }
